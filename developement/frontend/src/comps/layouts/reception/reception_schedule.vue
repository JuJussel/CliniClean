<template>
        ...
        <Column field="status" :header="$t('status')">
            <template #body="slotProps">
                ...
                <Select
                    ...
                    :options="examStatiOptions(slotProps.data.status)"
                    optionLabel="name"
                    optionValue="status"
                    @change="changeStatus($event, slotProps.data)"
                    class="w-full md:w-14rem"
                    :SelectChangeEvent="changeStatus"
                >
                    <template #value>
                        <span>
                            {{ parseStatusLabel(slotProps.data.status) }}
                        </span>
                    </template>
                </Select>
            </template>
    ...
    <Dialog
        v-model:visible="receptionAcceptModal.visible"
        modal
        :header="$t('reception') + $t('register')"
        class="w-3/5"
    >
        <ReceptionAcceptReservation
            v-bind:encounter="receptionAcceptModal.data"
            @close="receptionAcceptModal.visible = false"
            @commit="getSchedule()"
        />
    </Dialog>
</template>
...
function parseStatusLabel(status) {
    const statusData = listStore.listData.encounterStati.find(
        (item) => item[0].status === status
    );
    return statusData[0].name;
}

function examStatiOptions(currentStatus) {
    const encounterStati = listStore.listData.encounterStati;
