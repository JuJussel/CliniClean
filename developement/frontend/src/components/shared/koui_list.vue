      ...
      <vs-table v-bind:style="{ height: tableStyle }" ref="tbody" striped>
        ...
        <template #header>
          ...
          <div
            ...
            style="margin-bottom: 0; dispaly: flex"
          >
            <i
              :class="kouiCats[activeTab].icon"
              style="font-size: 30px; width: 60px"
            ></i>
            <vs-input
              type="search"
              state="dark"
              :placeholder="kouiCats[activeTab].name + '検索'"
              v-model="kouiCats[activeTab].search"
              @input="searchKoui(activeTab)"
            >
              <template #icon>
        ...
        <template #thead>
          ...
          <vs-th
            v-if="activeTab != 12 && activeTab !== '25'"
              ...
              <span slot-scope="{ node }">
                <template>
                  ...
                  <vs-tooltip shadow left v-else>
                    ...
                    <template #tooltip>
                      <kouiQuickView
                        v-for="(item, index) in node.data.content.items"
                        :key="index"
                        :koui="item"
                      />
                    </template>
...
export default {
  ...
  computed: {
    filderedKouiCats() {
      ...
      if (this.visibleCats) {
        arr = arr.filter((item) =>
          this.visibleCats.includes(parseInt(item.type))
        );
      } else {
  ...
  methods: {
    ...
    executeSearch(tab) {
      ...
      if (search !== "" && tab !== "12") {
        ...
        this.pendingSearch = this.$get("procedures", sendData, abortSignal)
          .then((result) => {
    ...
    getFavourites() {
      ...
      if (this.activeTab == 12) {
        ...
        this.$get(
          "favourites/" + this.$store.getters.userInfo.id,
          this.activeTab
        )
          .then((result) => {
            if (this.kouiCats[this.activeTab].search === "") {
