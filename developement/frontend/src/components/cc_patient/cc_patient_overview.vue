  ...
  <div class="cc-patient-list-main-cont">
    <cui-card>
      <patientList @selected="showPatientQuick">
        <template #buttons>
          <cui-button icon="fas fa-user-plus" :label="$lang.createNew" @click="newPatient" v-if="$aclService(2)"/>
        </template>
      </patientList>
    </cui-card>
    <cui-card noPadding>
      <patientInfo v-if="selectedPatientId" :patientId="selectedPatientId" ref="patientInfo">
        <template v-slot:buttons="{ patient }">
          <cui-button icon="fas fa-calendar-plus" :label="$lang.reservation" @click="walkinPatient(patient)" v-if="$aclService(2)"></cui-button>
          <cui-button icon="fas fa-walking" :label="$lang.reception" @click="walkinPatient(patient)" v-if="$aclService(2)"></cui-button>
          <cui-button icon="fas fa-info-circle" :label="$lang.details" @click="walkinPatient(patient)"></cui-button>
          <cui-button icon="fas fa-edit" :label="$lang.edit" @click="editPatient" v-if="$aclService(2)"></cui-button>
        </template>
      </patientInfo>
    </cui-card>
  </div>
