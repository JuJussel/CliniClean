...
const extensions = [Underline, TextStyle, Color, ImagePasteDropExtension, ResizableImage]

watch(() => props.modelValue, (val) => { content.value = val })
watch(content, (val) => emit('update:modelValue', val))

// Capture the editor instance so onClick handlers can reference it
    ...
    { kind: 'link', icon: 'i-lucide-link', tooltip: { text: 'Link' } },
    {
      ...
      onClick: () => {
        ...
        input.onchange = () => {
          ...
          if (file) insertImageFromFile(editorInstance.value, file)
        }
        input.click()
      }
    }
...
function onColorInput(event) {
  ...
  editorInstance.value?.chain().focus().setColor(selectedColor.value).run()
}
...
<template>
  <div >
    ...
    >
      <UEditorToolbar :editor="editor" :items="items"  class="border-b border-muted sticky top-0 inset-x-0 py-2 z-50 overflow-x-auto">
